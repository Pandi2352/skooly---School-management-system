...
| D2 | How does the NestJS API authenticate? | **Assumed: httpOnly session cookie.** `src/lib/api/client.ts` already sends `credentials: 'include'`. Confirm with the backend before building login. | Needs backend confirmation |
...
| Public routes | Blocked (D2) | `/login`, `/forgot-password` under an `AuthLayout` |
| Protected routes | Blocked (D2) | `RequireAuth` redirecting to `/login?redirect=<path>` |
| Auth handling | Blocked (D2) | |
| Nested routes | **Done** | |
